...
struct POI* filterPoints(const struct POI poiArray[], int numPoints,
                         const struct POI poi, double maxDistance, int* numSelected) {
  
  struct POI* selectedPOIs = (struct POI*) malloc(numPoints * sizeof(struct POI));
  if (!selectedPOIs) {
    *numSelected = 0;
  ...
  for (int i = 0; i < numPoints; i++) {
    double d = calcDistance(poi.lat, poi.lon, poiArray[i].lat, poiArray[i].lon);
    if (d < maxDistance) {
